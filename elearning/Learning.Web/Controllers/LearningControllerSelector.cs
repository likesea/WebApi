    ...
    public class LearningControllerSelector : DefaultHttpControllerSelector
    {
        ...
        public override HttpControllerDescriptor SelectController(HttpRequestMessage request)
        {
            var controllers = GetControllerMapping();
                //Will ignore any controls in same name even if they are in different namepsace
        ...
        //使用Accept Header设置版本
        private string GetVersionFromAcceptHeaderVersion(HttpRequestMessage requst)
        {
            var acceptHeader = requst.Headers.Accept;
            foreach (var mime in acceptHeader)
            {
                if (mime.MediaType == "application/json")
                {
                    var version =
                    mime.Parameters.Where(v => v.Name.Equals("version", StringComparison.OrdinalIgnoreCase))
                        .FirstOrDefault();
                    if (version != null)
                    {
